...
namespace LOVA.API.Pages.Lottingelund
{
    public class LeafletModel : PageModel
    {
        ...
        public LeafletModel(LovaDbContext context, UserManager<ApplicationUser> userManager, IBlobService blobService)
        {
            _context = context;
        ...
        public async Task<JsonResult> OnPostRead(string target)
        {
            ...
            var lovaFiles = await _context.UploadedFiles
                .Where(a => a.UploadFileCategoryId == 5 && a.IsDirectory == false && a.Directory == target)
                .Select(a => new ViewFilesViewModel
                {
